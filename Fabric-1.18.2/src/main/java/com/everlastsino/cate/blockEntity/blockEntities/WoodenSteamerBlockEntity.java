...
public class WoodenSteamerBlockEntity extends BlockEntity {
            ...
            if(blockEntity instanceof WoodenSteamerBlockEntity entity){
                ...
                for(int i = 0; i < 4; ++i){
                    if(entity.downItems.get(i).isEmpty()){
                        entity.downItems.set(i, itemStack.copy());
                        entity.downItems.set(i + 4, itemBeingCooked.copy());
                ...
                if(entity.enableUp){
                    for(int i = 0; i < 4; ++i){
                        if(entity.upItems.get(i).isEmpty()){
                            entity.upItems.set(i, itemStack.copy());
                            entity.upItems.set(i + 4, itemBeingCooked.copy());
        ...
        while(this.world != null && floors >= 0) {
            ...
            if(blockEntity instanceof WoodenSteamerBlockEntity entity){
                if(entity.enableUp){
                    for(int i = 3; i >= 0; --i){
                        if(!entity.upItems.get(i).isEmpty()){
                            ItemStack itemStack = entity.upItems.get(i).copy();
                            if(isPick) {
                ...
                for(int i = 3; i >= 0; --i){
                    if(!entity.downItems.get(i).isEmpty()){
                        ItemStack itemStack = entity.downItems.get(i).copy();
                        if(isPick) {
            ...
            if(entity.canLit){
                ...
                for (int i = 0; i < 4 && i < entity.upTimes.length && i < entity.downTimes.length; ++i) {
                    if (entity.upTimes[i] <= 0) {
                        entity.upItems.set(i, entity.upItems.get(i + 4).copy());
                    } else {
                        entity.upTimes[i]--;
                        isLit = true;
                    }
                    if (entity.downTimes[i] <= 0) {
                        entity.downItems.set(i, entity.downItems.get(i + 4).copy());
                    } else {
                        entity.downTimes[i]--;
                        isLit = true;
                    }
    ...
    public static <T extends BlockEntity> void clientTick(World world, BlockPos blockPos, BlockState blockState, T t) {
        if(world != null && t instanceof WoodenSteamerBlockEntity entity){
            if(entity.canLit){
                Random random = new Random();
